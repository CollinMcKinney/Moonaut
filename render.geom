...
/* ---- Outputs ---- */
flat out vec3 gFlatColor;      // flat shaded colour (constant per triangle)
out vec3 gVertexColor;         // Gouraud per‑vertex colours (interpolated)
out vec3 gWorldPos;
out vec3 gNormal;
out vec3 gLocalPos;
flat out int gMode;            // shading mode (passed from uniform)

void main() {
    // Compute face normal (flat shading) from world-space vertices
    vec3 faceNormal = normalize(cross(
        vWorldPos[1] - vWorldPos[0],
        vWorldPos[2] - vWorldPos[0]
    ));
    // Ensure normal points towards camera
    vec3 centerPos = (vWorldPos[0] + vWorldPos[1] + vWorldPos[2]) / 3.0;
    vec3 V = normalize(uCamEye - centerPos);
    if (dot(faceNormal, V) < 0.0) faceNormal = -faceNormal;

    // Compute flat colour using face normal
    vec3 centerLocal = (vLocalPos[0] + vLocalPos[1] + vLocalPos[2]) / 3.0;
    gFlatColor = shade_surface(faceNormal, centerPos);

    // Compute per‑vertex colours for Gouraud
    vec3 vertexColors[3];
    vertexColors[0] = shade_surface(normalize(vNormal[0]), vWorldPos[0]);
    vertexColors[1] = shade_surface(normalize(vNormal[1]), vWorldPos[1]);
    vertexColors[2] = shade_surface(normalize(vNormal[2]), vWorldPos[2]);
    ...
    for (int i = 0; i < 3; i++) {
        ...
        gNormal = vNormal[i];
        gLocalPos = vLocalPos[i];
        gVertexColor = vertexColors[i];
        EmitVertex();
    }
    EndPrimitive();
}