...
static INLINE vec4 quat_to_axis_angle(vec4 quaternion)
{
    ...
    real sin_half = real_sqrt(real_max(0.0f, 1.0f - (normalized.quaternion.real * normalized.quaternion.real)));
    ...
    return vec4_init_from_4(
        normalized.quaternion.imaginary.rotation.i / sin_half,
        normalized.quaternion.imaginary.rotation.j / sin_half,
        normalized.quaternion.imaginary.rotation.k / sin_half,
        half_angle * 2.0f);
}
...
static INLINE vec4 quat_between_vec3(vec3 from, vec3 to)
{
    ...
    quaternion = vec4_init_from_4(axis.rotation.i, axis.rotation.j, axis.rotation.k, 1.0f + dot);
    return vec4_normalize(quaternion);
}
...
static INLINE vec3 quat_rotate_vec3(vec4 quaternion, vec3 vector)
{
    ...
    /* TODO: split this into multiple lines with meaningful variable names. */
    vec3 rotated = vec3_add(vector, vec3_mul_scalar(vec3_add(vec3_mul_scalar(uv, normalized.quaternion.real), uuv), 2.0f));
    return rotated;
}
